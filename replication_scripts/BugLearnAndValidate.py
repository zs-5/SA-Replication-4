...
@author: Michael Pradel, Sabine 

Modified by Zavier on Apr 19, 2026 to:
    - Output time taken for extracting xy pairs.
    - Save timings to text file (outputs/training_prediction_time.txt)
    - Save threshold and recall values to csv file (outputs/threshold_recalls.csv) for use in graph_recall.py
    - Fix Tensorflow errors from using internal libs instead of public ones.
    - Fix error saving model by appending .keras file extension.
'''
...
# Z: Disabled because it results in the following error, due to importing from internal libs (tensorflow.python.keras) instead of the public lib/API (tensorflow.keras)
...
def prepare_xy_pairs(gen_negatives, data_paths, learning_data):
    ...
    # Z
    time_extracting_pairs = round(time.time() - time_extracting_pairs_start)
    with open("outputs/training_prediction_time.txt", "a") as file:
        file.write(f"- Extraction: {time_extracting_pairs//60}:{time_extracting_pairs%60:02d}\n")

    print("Time for extracting pairs (seconds):", time_extracting_pairs) # Z
...
if __name__ == '__main__':
    ...
    model.save(f"bug_detection_model_{time_stamp}.keras")
    # Z: Added ".keras" file extension to model.save to solve following error:
    # ValueError: Invalid filepath extension for saving. Please add either a `.keras` extension for the native Keras format (recommended) or a `.h5` extension. Use `model.export(filepath)` if you want to export a SavedModel for use with TFLite/TFServing/etc. Received: filepath=bug_detection_model_1776680398331.
...
#             print("Possible anomaly: "+message)
    ...
    with open("outputs/threshold_recalls.csv", "w") as csvfile: # Z: To save threshold and recall values in csv
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(["Threshold", "Recall"])
        ...
        for threshold_raw in range(1, 20, 1):
            ...
            if 0.5 <= threshold <= 0.9: # Z
                csvwriter.writerow([threshold, round(recall, 4)])